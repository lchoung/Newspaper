...
- (void)layoutSubviews {
    ...
    if (self.zoomScale - self.minimumZoomScale > 0.1) {
        // Horizontally
        if (frameToCenter.size.width  * self.zoomScale < boundsSize.width) {
            frameToCenter.origin.x = floorf((boundsSize.width - frameToCenter.size.width) / 2.0);
        } else {
            frameToCenter.origin.x = 0;
        }
        ...
        // Vertically
        if (frameToCenter.size.height * self.zoomScale < boundsSize.height) {
            frameToCenter.origin.y = floorf((boundsSize.height - frameToCenter.size.height) / 2.0);
        } else {
            frameToCenter.origin.y = 0;
        }
...
- (void)setMaxMinZoomScalesForCurrentBounds
{
	...
	// Calculate Max
	CGFloat maxScale = 2 * scaleFactor; // Allow double scale
