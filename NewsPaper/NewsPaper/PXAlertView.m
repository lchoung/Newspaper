...
- (id)initWithTitle:(NSString *)title
        ...
        contentView:(UITextField *)contentView
         ...
         completion:(PXAlertViewCompletionBlock)completion
{
    ...
    if (self) {
        ...
        self.alertView.backgroundColor = BARTEXTCOLOR;
        ...
        self.titleLabel.font = ALERTTITLE;
        ...
        if (message.length < 1) {
            messageLabelY = self.titleLabel.frame.origin.y;
            self.messageLabel.frame = CGRectMake(AlertViewContentMargin,
                                                  2* AlertViewVerticalElementSpace,
                                                 AlertViewWidth - AlertViewContentMargin*2,
                                                 0);
        }
        CGFloat contentViewShift = 0;
        // Optional Content View
        if (contentView) {
            self.alertView.layer.opacity = 1;
            contentViewShift = AlertViewVerticalElementSpace / 2;
            self.contentView = contentView;
            self.contentView.layer.cornerRadius = 5;
            self.contentView.clipsToBounds = YES;
            self.contentView.frame = CGRectMake(AlertViewContentMargin, messageLabelY + self.messageLabel.frame.size.height + AlertViewVerticalElementSpace, AlertViewWidth - AlertViewContentMargin*2, 30);
            self.contentView.backgroundColor = ACCENTCOLOR;
            self.contentView.opaque = YES;
            self.contentView.font = TEXTFONT;
            self.contentView.textColor = TEXTCOLOR;
            self.contentView.tintColor = TEXTCOLOR;
            self.contentView.keyboardAppearance = KEYBOARD;
            [self.alertView addSubview:self.contentView];
        }
        // Line
        CALayer *lineLayer = [self lineLayer];
        lineLayer.frame = CGRectMake(0, self.messageLabel.frame.origin.y + self.messageLabel.frame.size.height + 2 * AlertViewVerticalElementSpace + self.contentView.frame.size.height + contentViewShift, AlertViewWidth, AlertViewLineLayerWidth);
        [self.alertView.layer addSublayer:lineLayer];
        
        self.buttonsY = lineLayer.frame.origin.y + lineLayer.frame.size.height - AlertViewVerticalElementSpace / 2 + contentViewShift;
...
- (void)resizeViews
{
    ...
    if (self.messageLabel.text.length < 1) {
        totalHeight -= 22;
    }
    CGRect screenRect = [[UIScreen mainScreen] bounds];
    CGFloat screenWidth = screenRect.size.width;
