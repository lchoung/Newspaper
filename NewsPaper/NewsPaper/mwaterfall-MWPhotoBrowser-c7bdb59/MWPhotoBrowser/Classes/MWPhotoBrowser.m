...
@property (nonatomic) NSNumber *hasPosts;
@end
...
- (id)initWithDelegate:(id <MWPhotoBrowserDelegate>)delegate hasPosts:(BOOL)hasPosts
{
    self = [self initWithDelegate:delegate];
    self.hasPosts = [NSNumber numberWithBool:hasPosts];
    return self;
}
...
- (void)viewDidLoad {
    ...
    _toolbar.tintColor = ACCENTCOLOR;
    _toolbar.backgroundColor = ACCENTCOLOR;
...
- (void)performLayout {
    ...
    [_actionButton setTitleTextAttributes:TITLEATTRIBUTES forState:UIControlStateNormal];
    [_actionButton setTitleTextAttributes:TITLEATTRIBUTES forState:UIControlStateHighlighted];
    // Toolbar visibility
...
#pragma clang diagnostic push
    ...
    if (self.hasPosts == [NSNumber numberWithBool:NO])
    {
        CGRect placeHolderFrame = [[UIScreen mainScreen] applicationFrame];
        UIImageView *placeHolderView = [[UIImageView alloc]initWithFrame: placeHolderFrame];
        placeHolderView.image = [UIImage imageNamed:@"placeHolderGallery"];
...
- (void)updateNavigation {
    ...
    NSLog(@"%@",[self photoAtIndex:_currentPageIndex]);
